...
output = p1.communicate()[0] # class 'bytes', sequence of bytes, similar to string, but ASCII only and immutable
...
output_dict = dict(item.split(": ") for item in output_list[:-1]) # the last item is an ' and can't be split
...
def get_media_properties(filename): # hachoir not the way to go, use exiftools intead (see above)
